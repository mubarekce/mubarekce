...
const LiveStreams: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#fdfdfd] animate-in fade-in duration-500 overflow-hidden relative">
      {/* Background Glow */}
      <div className="absolute top-0 left-0 w-full h-80 bg-gradient-to-b from-emerald-50/50 to-transparent pointer-events-none -z-10"></div>
      ...
      <div className="px-5 pt-12 pb-4 flex items-center justify-between bg-white/80 backdrop-blur-xl sticky top-0 z-40 border-b border-slate-100/50">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[8px] font-black text-emerald-600 uppercase tracking-[0.25em] mt-1">Haremeyn-i Şerifeyn</p>
          </div>
        </div>
        <div className="w-10 h-10 bg-emerald-50 rounded-xl flex items-center justify-center text-lg border border-emerald-100 text-emerald-600 animate-pulse">📡</div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 pb-32 no-scrollbar pt-6 space-y-8">
        ...
        <div className="bg-slate-100/60 p-1.5 rounded-[2rem] flex border border-slate-200/50 shadow-inner">
          {STREAMS.map(stream => (
            <button 
              ...
              className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-[1.6rem] transition-all duration-300 flex items-center justify-center gap-2 ${activeTab === stream.id ? 'bg-emerald-600 text-white shadow-lg border border-emerald-500' : 'text-slate-400 hover:text-slate-600'}`}
            >
        ...
        <div className="bg-white rounded-[2.8rem] p-8 border border-slate-100 shadow-xl shadow-slate-900/5 space-y-6 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-4">
             <div className="space-y-1">
                ...
                <p className="text-[10px] font-black text-emerald-600 uppercase tracking-[0.4em]">{currentStream.subTitle}</p>
             </div>
             ...
             <div className="pt-4 grid grid-cols-2 gap-3">
                <div className="bg-emerald-50/50 p-5 rounded-3xl border border-emerald-100/50 text-center hover:bg-emerald-50 transition-colors">
                   <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-1.5">MANEVİ NOT</p>
        ...
        {/* Prayer Quote Card */}
        <div className="bg-emerald-950 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-emerald-950/20 text-center">
           ...
           <div className="relative z-10 space-y-4">
              <p className="text-emerald-400 text-[9px] font-black uppercase tracking-[0.6em]">TEFEKKÜR VAKTİ</p>
              ...
              <p className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">HAYIRLI TEFEKKÜRLER</p>
           </div>
        </div>
