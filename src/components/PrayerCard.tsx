...
const PrayerCard: React.FC<PrayerCardProps> = ({ name, time, isActive, icon }) => {
  return (
    <div className={`group relative px-6 py-5 rounded-[2.2rem] flex items-center justify-between transition-all duration-500 border cursor-pointer ${
      isActive 
        ? 'bg-white dark:bg-slate-900 border-teal-500 shadow-xl shadow-teal-900/5 ring-1 ring-teal-500 scale-[1.03] z-10' 
        : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 hover:border-teal-100 hover:bg-slate-50/50 dark:bg-slate-900/50'
    }`}>
      <div className="flex items-center gap-6">
        <div className={`w-14 h-14 rounded-3xl flex items-center justify-center transition-all duration-500 ${
          isActive 
            ...
            : 'bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-500 group-hover:bg-teal-50 dark:bg-teal-950/20 group-hover:text-teal-600'
        }`}>
          <span className="text-2xl group-hover:scale-110 transition-transform">{icon}</span>
        </div>
        <div>
          <p className={`text-[9px] font-black uppercase tracking-[0.25em] mb-1 ${isActive ? 'text-teal-600' : 'text-slate-300 dark:text-slate-600'}`}>
            {name}
          </p>
          <p className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">{time}</p>
        </div>
      </div>
