...
const PrayerCard: React.FC<PrayerCardProps> = ({ name, time, isActive, icon }) => {
  return (
    <div className={`group relative px-6 py-5 rounded-[2.2rem] flex items-center justify-between transition-all duration-500 border cursor-pointer ${
      isActive 
        ? 'bg-white border-emerald-500 shadow-xl shadow-emerald-900/5 ring-1 ring-emerald-500 scale-[1.03] z-10' 
        : 'bg-white border-slate-100 hover:border-emerald-100 hover:bg-slate-50/50'
    }`}>
      <div className="flex items-center gap-6">
        ...
        <div>
          <p className={`text-[9px] font-black uppercase tracking-[0.25em] mb-1 ${isActive ? 'text-emerald-600' : 'text-slate-300'}`}>
            {name}
          </p>
      ...
      {isActive ? (
        <div className="flex flex-col items-end gap-1">
          <span className="px-3 py-1 bg-emerald-600 text-white rounded-full text-[8px] font-black uppercase tracking-widest shadow-md">Aktif</span>
          <span className="text-[9px] font-bold text-emerald-600/40 uppercase tracking-tighter">Vakit Çıkıyor</span>
        </div>
      ) : (
