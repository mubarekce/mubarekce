      ...
      <div className="fixed inset-0 z-[260] bg-white dark:bg-navy-800 flex flex-col animate-in slide-in-from-right duration-500 overflow-hidden">
        <div className="px-6 pt-12 pb-6 border-b border-slate-50 dark:border-navy-900 flex items-center justify-between bg-white dark:bg-navy-800 sticky top-0 z-20">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-[17px] font-black text-slate-900 dark:text-white tracking-tight leading-tight uppercase">{selectedHalkala.name}</h2>
              <p className="text-[9px] font-black text-gold-500 uppercase tracking-[0.2em] mt-0.5">Cüz Dağıtım Tablosu</p>
            </div>
          </div>
          <button 
            onClick={() => setShowParticipantsModal(true)}
            className="w-10 h-10 bg-gold-50 dark:bg-navy-950/20 rounded-2xl flex items-center justify-center text-gold-600 border border-gold-100 active:scale-90 transition-transform"
          >
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-32 no-scrollbar">
          <div className="mt-8 mb-10 p-8 bg-gold-50/50 rounded-[2.8rem] border border-gold-100 relative overflow-hidden">
             <div className="absolute top-[-20%] right-[-10%] opacity-5 rotate-12 text-[12rem] pointer-events-none">📖</div>
             <div className="relative z-10">
                <div className="flex justify-between items-end mb-5">
                   <div className="space-y-1">
                      <p className="text-[10px] font-black text-gold-600 uppercase tracking-widest">İLERLEME DURUMU</p>
                      <h4 className="text-[2.8rem] font-black text-navy-950 leading-none tracking-tighter">%{percent}</h4>
                   </div>
                   <div className="text-right pb-1">
                      <p className="text-[9px] font-black text-gold-400 uppercase tracking-widest mb-1">HEDEF TARİH</p>
                      <p className="text-xs font-black text-navy-800">{selectedHalkala.deadline}</p>
                   </div>
                </div>
                ...
                <button 
                  ...
                  className={`aspect-square rounded-[1.2rem] flex flex-col items-center justify-center border-2 transition-all active:scale-90 overflow-hidden ${
                    assignment.status === 'completed' 
                      ? 'bg-gold-600 border-gold-500 text-white shadow-lg shadow-navy-900/10' 
                      : assignment.isMe
        ...
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white/95 to-transparent pb-[calc(1.5rem+env(safe-area-inset-bottom))]">
          <button 
            ...
            className="w-full py-5 bg-gold-700 text-white rounded-[2rem] font-black text-[11px] uppercase tracking-[0.25em] shadow-xl shadow-navy-900/10 active:scale-95 transition-all hover:bg-navy-800"
          >
            HALKAYA DAVET ET
          </button>
        </div>
    ...
    <div className="flex-1 flex flex-col bg-[#FCFDFD] h-full relative animate-in fade-in duration-500 overflow-hidden">
      ...
      <div className="px-6 pt-12 pb-6 flex items-center justify-between bg-white dark:bg-navy-800 border-b border-slate-100/50 sticky top-0 z-20">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight leading-none uppercase">Hatim Organizatörü</h2>
            <p className="text-[9px] font-black text-gold-500 uppercase tracking-[0.2em] mt-1">Grup Hatim Yönetimi</p>
          </div>
        </div>
        <button onClick={() => setShowCreateModal(true)} className="w-10 h-10 bg-gold-50 dark:bg-navy-950/20 rounded-2xl flex items-center justify-center text-gold-600 border border-gold-100 shadow-sm active:scale-90 transition-transform">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-32 no-scrollbar">
        {activeTab === 'halkalar' ? (
          <div className="py-6 space-y-8 animate-in fade-in duration-500">
            <div className="bg-[#0a1f1a] rounded-[2.8rem] p-10 text-white relative overflow-hidden group shadow-[0_25px_60px_-15px_rgba(10,31,26,0.3)]">
                ...
                halkalar.map((halkala) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-6">
                       <div className="space-y-1">
                          <h4 className="text-[16px] font-black text-slate-950 group-hover:text-gold-700 transition-colors tracking-tight">{halkala.name}</h4>
                          <div className="flex items-center gap-2">
                             <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Kuran Hatmi</p>
                             <span className="w-1 h-1 rounded-full bg-slate-200"></span>
                             <p className="text-[9px] font-black text-gold-500 uppercase tracking-widest">{(Object.values(halkala.assignments) as JuzAssignment[]).filter(a => a.participantName).length} Katılımcı</p>
                          </div>
                       </div>
                       <div className="w-11 h-11 bg-gold-50 dark:bg-navy-950/20 rounded-2xl flex items-center justify-center text-gold-600 transition-all group-hover:translate-x-1 shadow-sm border border-gold-100/50">
                    ...
                    <div className="flex items-center gap-5">
                       ...
                       <span className="text-[11px] font-black text-gold-600 tabular-nums">%{Math.round((halkala.completedParts / halkala.totalParts) * 100)}</span>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="py-8 space-y-6 animate-in fade-in duration-500">
             ...
             <div className="bg-navy-900 rounded-[2.8rem] p-8 text-white relative overflow-hidden shadow-xl shadow-navy-950/20">
                <div className="absolute right-0 bottom-0 p-6 opacity-10 text-7xl">📈</div>
                <div className="relative z-10 space-y-6">
                   <div>
                      <p className="text-[10px] font-black text-gold-400 uppercase tracking-widest mb-1">ŞAHSİ TAMAMLANAN</p>
                      <h3 className="text-4xl font-black tracking-tighter">{stats.myCompletedJuz} <span className="text-lg text-gold-500/60 font-bold tracking-normal">/ {stats.myJuzCount} CÜZ</span></h3>
                   </div>
                   ...
                   <p className="text-[10px] font-medium text-gold-300/80 leading-relaxed italic">"İki günü eşit olan ziyandadır." - Tebrikler, manevi yolculuğun devam ediyor!</p>
                </div>
             </div>
                ...
                {halkalar.length === 0 ? (
                  ...
                  halkalar.map(h => (
                    <div key={h.id} className="flex items-center justify-between py-2">
                       <span className="text-xs font-bold text-slate-700 dark:text-slate-300 dark:text-slate-600">{h.name}</span>
                       <span className="text-xs font-black text-gold-600">%{Math.round((h.completedParts / 30) * 100)}</span>
                    </div>
                  ))
                )}
             </div>
          ...
          <div className="bg-white dark:bg-navy-800 w-full max-w-sm rounded-[3rem] p-10 space-y-8 animate-in zoom-in duration-300 shadow-2xl">
             ...
             <div className="space-y-6">
                <div className="space-y-2">
                   <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-4">HALKA ADI</label>
                   <input 
                     type="text" 
                     value={newHalkaName} 
                ...
                <div className="space-y-2">
                   <div className="flex justify-between items-center ml-4">
                      <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">HEDEF TARİH</label>
                      <div className="flex items-center gap-2">
                         <input 
                           type="checkbox" 
                           id="noDate" 
                           ...
                           className="w-3 h-3 rounded border-slate-300 text-gold-600 focus:ring-gold-500" 
                         />
                         <label htmlFor="noDate" className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest cursor-pointer">TARİH YOK</label>
                      </div>
                   </div>
                   <input 
                     type="date" 
        ...
        <div className="fixed inset-0 z-[450] bg-slate-900/70 backdrop-blur-md flex items-end justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white dark:bg-navy-800 w-full max-w-sm rounded-[3rem] p-10 space-y-8 animate-in slide-in-from-bottom-20 duration-500 shadow-2xl">
              <div className="text-center space-y-2">
                 ...
                 <h3 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">{showActionMenu.juzIndex + 1}. Cüz İşlemleri</h3>
                 <div className="flex items-center justify-center gap-2">
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">OKUYAN:</p>
                    <p className="text-[11px] font-black text-gold-600 uppercase tracking-widest">{showActionMenu.assignment.participantName}</p>
                 </div>
              </div>
              ...
              <div className="space-y-3">
                 <button 
                   ...
                   className="w-full py-5 bg-gold-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.25em] shadow-lg shadow-gold-200 active:scale-95 transition-all flex items-center justify-center gap-3"
                 >
                 ...
                 <button 
                   ...
                   className="w-full py-5 bg-rose-50 dark:bg-rose-950/20 text-rose-500 border border-rose-100 rounded-2xl font-black text-[11px] uppercase tracking-[0.25em] active:scale-95 transition-all flex items-center justify-center gap-3"
                 >
        ...
        <div className="fixed inset-0 z-[400] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white dark:bg-navy-800 w-full max-w-sm rounded-[3rem] p-10 space-y-8 animate-in zoom-in duration-300 shadow-2xl max-h-[85vh] flex flex-col">
              ...
              <div className="space-y-4">
                 <div className="space-y-2">
                    <label className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-4">KATILIMCI ADI</label>
                    <div className="flex gap-2">
                       ...
                       <button 
                         ...
                         className="w-14 h-14 bg-gold-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-gold-200 active:scale-90 transition-transform"
                       >
              ...
              <div className="flex-1 overflow-y-auto pr-2 no-scrollbar space-y-3">
                 ...
                 {selectedHalkala.participants.length === 0 && (
                   <p className="text-center text-xs text-slate-400 dark:text-slate-500 py-10">Henüz kimse eklenmemiş.</p>
                 )}
              </div>
        ...
        <div className="fixed inset-0 z-[400] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white dark:bg-navy-800 w-full max-w-sm rounded-[3rem] p-10 space-y-8 shadow-2xl text-center">
            ...
            <div className="bg-gold-50 dark:bg-navy-950/20 border-2 border-dashed border-gold-200 rounded-[2rem] py-8 px-4">
              <p className="text-4xl font-black text-gold-700 tracking-[0.3em]">{selectedHalkala.inviteCode}</p>
            </div>
            <div className="flex gap-4">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-white via-white/80 to-transparent pb-[calc(1.5rem+env(safe-area-inset-bottom))] text-center pointer-events-none z-10">
        <p className="text-[8px] font-black text-slate-300 dark:text-slate-600 uppercase tracking-[0.65em]">Grup İbadet Platformu V1.2</p>
      </div>
    </div>
  );
};
