    ...
    return (
      <div className="fixed inset-0 z-[260] bg-white flex flex-col animate-in slide-in-from-right duration-500 overflow-hidden">
        <div className="px-6 pt-12 pb-6 border-b border-slate-50 flex items-center justify-between bg-white sticky top-0 z-20">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mt-0.5">Cüz Dağıtım Tablosu</p>
            </div>
          </div>
          <button 
            onClick={() => setShowParticipantsModal(true)}
            className="w-10 h-10 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 border border-emerald-100 active:scale-90 transition-transform"
          >
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-32 no-scrollbar">
          <div className="mt-8 mb-10 p-8 bg-emerald-50/50 rounded-[2.8rem] border border-emerald-100 relative overflow-hidden">
             <div className="absolute top-[-20%] right-[-10%] opacity-5 rotate-12 text-[12rem] pointer-events-none">📖</div>
             <div className="relative z-10">
                <div className="flex justify-between items-end mb-5">
                   <div className="space-y-1">
                      <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">İLERLEME DURUMU</p>
                      <h4 className="text-[2.8rem] font-black text-emerald-950 leading-none tracking-tighter">%{percent}</h4>
                   </div>
                   <div className="text-right pb-1">
                      <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-1">HEDEF TARİH</p>
                      <p className="text-xs font-black text-emerald-800">{selectedHalkala.deadline}</p>
                   </div>
                </div>
                <div className="w-full h-3 bg-white/60 rounded-full overflow-hidden border border-emerald-100/50 shadow-inner">
                   <div 
                    ...
                    assignment.status === 'completed' 
                      ...
                      : assignment.isMe
                        ...
                        : assignment.status === 'reserved' 
                          ...
                          : 'bg-white border-slate-50 text-slate-300 hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-600'
                  }`}
                >
        ...
        <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white/95 to-transparent pb-[calc(1.5rem+env(safe-area-inset-bottom))]">
          <button 
            ...
            className="w-full py-5 bg-emerald-700 text-white rounded-[2rem] font-black text-[11px] uppercase tracking-[0.25em] shadow-xl shadow-emerald-900/10 active:scale-95 transition-all hover:bg-emerald-800"
          >
            HALKAYA DAVET ET
          </button>
        </div>
      ...
      <div className="px-6 pt-12 pb-6 flex items-center justify-between bg-white border-b border-slate-100/50 sticky top-0 z-20">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] mt-1">Grup Hatim Yönetimi</p>
          </div>
        </div>
        <button onClick={() => setShowCreateModal(true)} className="w-10 h-10 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 border border-emerald-100 shadow-sm active:scale-90 transition-transform">
      ...
      <div className="px-6 pt-6 pb-2">
        <div className="bg-slate-100/80 p-1.5 rounded-[1.8rem] flex border border-slate-100 shadow-sm">
          <button 
            ...
            className={`flex-1 py-3.5 rounded-[1.4rem] text-[10px] font-black uppercase tracking-widest transition-all relative overflow-hidden ${activeTab === 'halkalar' ? 'bg-white text-emerald-700 shadow-md ring-1 ring-emerald-500/10' : 'text-slate-400'}`}
          >
            Halkalarım
          </button>
          <button 
            onClick={() => setActiveTab('istatistik')}
            className={`flex-1 py-3.5 rounded-[1.4rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'istatistik' ? 'bg-white text-emerald-700 shadow-md ring-1 ring-emerald-500/10' : 'text-slate-400'}`}
          >
            İstatistikler
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-32 no-scrollbar">
        {activeTab === 'halkalar' ? (
          <div className="py-6 space-y-8 animate-in fade-in duration-500">
            <div className="bg-[#052e16] rounded-[2.8rem] p-10 text-white relative overflow-hidden group shadow-[0_25px_60px_-15px_rgba(5,46,22,0.3)]">
               ...
               <div className="relative z-10">
                  ...
                  <p className="text-emerald-400/80 text-[10px] font-black uppercase tracking-[0.25em] mb-10 leading-relaxed">Sevdiklerinle beraber<br/>tilavetin bereketini paylaş</p>
                  <button onClick={() => setShowCreateModal(true)} className="bg-emerald-500 hover:bg-emerald-400 text-white font-black text-[10px] px-10 py-4.5 rounded-[1.5rem] uppercase tracking-widest shadow-[0_15px_30px_rgba(16,185,129,0.3)] transition-all active:scale-95 border border-emerald-400/20">
                    Halka Oluştur
                  </button>
               </div>
            </div>
            ...
            <div className="space-y-5">
              <div className="flex items-center gap-3 ml-2 mb-3">
                 <div className="w-2 h-2 bg-emerald-500 rounded-full shadow-[0_0_12px_rgba(16,185,129,0.7)] animate-pulse"></div>
              ...
              {halkalar.length === 0 ? (
                ...
                halkalar.map((halkala) => (
                  <div 
                    ...
                    className="bg-white p-7 rounded-[2.8rem] border border-slate-100 shadow-[0_12px_45px_-12px_rgba(0,0,0,0.06)] hover:border-emerald-100 transition-all cursor-pointer group active:scale-[0.98] relative overflow-hidden"
                  >
                    <div className="flex justify-between items-start mb-6">
                       <div className="space-y-1">
                          <h4 className="text-[16px] font-black text-slate-950 group-hover:text-emerald-700 transition-colors tracking-tight">{halkala.name}</h4>
                          <div className="flex items-center gap-2">
                             <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Kuran Hatmi</p>
                             <span className="w-1 h-1 rounded-full bg-slate-200"></span>
                             <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">{(Object.values(halkala.assignments) as JuzAssignment[]).filter(a => a.participantName).length} Katılımcı</p>
                          </div>
                       </div>
                       <div className="w-11 h-11 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-600 transition-all group-hover:translate-x-1 shadow-sm border border-emerald-100/50">
                    ...
                    <div className="flex items-center gap-5">
                       ...
                       <span className="text-[11px] font-black text-emerald-600 tabular-nums">%{Math.round((halkala.completedParts / halkala.totalParts) * 100)}</span>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="py-8 space-y-6 animate-in fade-in duration-500">
             <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm text-center space-y-1">
                   ...
                   <p className="text-3xl font-black text-emerald-700 tracking-tighter">{stats.totalJuz}</p>
                </div>
                <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm text-center space-y-1">
                   <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">AKTİF HALKA</p>
                   <p className="text-3xl font-black text-emerald-700 tracking-tighter">{stats.activeCircles}</p>
                </div>
             </div>

             <div className="bg-emerald-900 rounded-[2.8rem] p-8 text-white relative overflow-hidden shadow-xl shadow-emerald-950/20">
                <div className="absolute right-0 bottom-0 p-6 opacity-10 text-7xl">📈</div>
                <div className="relative z-10 space-y-6">
                   <div>
                      <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1">ŞAHSİ TAMAMLANAN</p>
                      <h3 className="text-4xl font-black tracking-tighter">{stats.myCompletedJuz} <span className="text-lg text-emerald-500/60 font-bold tracking-normal">/ {stats.myJuzCount} CÜZ</span></h3>
                   </div>
                   ...
                   <p className="text-[10px] font-medium text-emerald-300/80 leading-relaxed italic">"İki günü eşit olan ziyandadır." - Tebrikler, manevi yolculuğun devam ediyor!</p>
                </div>
             </div>
                ...
                {halkalar.length === 0 ? (
                  ...
                  halkalar.map(h => (
                    <div key={h.id} className="flex items-center justify-between py-2">
                       ...
                       <span className="text-xs font-black text-emerald-600">%{Math.round((h.completedParts / 30) * 100)}</span>
                    </div>
                  ))
                )}
             </div>
        ...
        <div className="fixed inset-0 z-[400] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 space-y-8 animate-in zoom-in duration-300 shadow-2xl">
             <div className="text-center space-y-1">
                ...
                <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Halkayı Kur ve Başlat</p>
             </div>
             ...
             <div className="flex gap-4">
                ...
                <button onClick={handleCreateHalka} className="flex-1 py-4 bg-emerald-600 text-white font-black rounded-2xl text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-200 active:scale-95 transition-all">KAYDET</button>
             </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[400] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 space-y-8 animate-in zoom-in duration-300 shadow-2xl max-h-[80vh] flex flex-col">
             <div className="text-center space-y-1">
                ...
                <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Hoca / Katılımcı Seçin</p>
             </div>
             ...
             <div className="flex-1 overflow-y-auto pr-2 no-scrollbar space-y-3">
                ...
                {selectedHalkala.participants.map((p, idx) => (
                  <button 
                    ...
                    className="w-full flex items-center justify-between p-5 bg-slate-50 border border-slate-100 rounded-2xl group active:scale-[0.98] transition-all hover:bg-emerald-50 hover:border-emerald-200"
                  >
                     <div className="flex items-center gap-4">
                        <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-xs font-bold text-emerald-600 shadow-sm border border-slate-100">{p[0]}</div>
                        <span className="font-bold text-sm text-slate-700">{p}</span>
                     </div>
                     <span className="text-[10px] font-black text-emerald-600 opacity-0 group-hover:opacity-100 transition-all uppercase tracking-widest">SEÇ</span>
                  </button>
                ))}
        ...
        <div className="fixed inset-0 z-[450] bg-slate-900/70 backdrop-blur-md flex items-end justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 space-y-8 animate-in slide-in-from-bottom-20 duration-500 shadow-2xl">
              <div className="text-center space-y-2">
                 <div className="w-16 h-16 bg-emerald-50 rounded-2xl flex items-center justify-center mx-auto text-3xl shadow-inner border border-emerald-100">
                 ...
                 <div className="flex items-center justify-center gap-2">
                    ...
                    <p className="text-[11px] font-black text-emerald-600 uppercase tracking-widest">{showActionMenu.assignment.participantName}</p>
                 </div>
              </div>
              ...
              <div className="space-y-3">
                 <button 
                   ...
                   className="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black text-[11px] uppercase tracking-[0.25em] shadow-lg shadow-emerald-200 active:scale-95 transition-all flex items-center justify-center gap-3"
                 >
        ...
        <div className="fixed inset-0 z-[400] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-sm rounded-[3rem] p-10 space-y-8 animate-in zoom-in duration-300 shadow-2xl max-h-[85vh] flex flex-col">
              <div className="text-center space-y-1">
                 ...
                 <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Halka İçin Havuz Oluşturun</p>
              </div>
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                       ...
                       <button 
                         ...
                         className="w-14 h-14 bg-emerald-600 text-white rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-200 active:scale-90 transition-transform"
                       >
                 ...
                 {selectedHalkala.participants.map((p, idx) => (
                   <div key={idx} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                      <div className="flex items-center gap-3">
                         <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-[10px] font-black text-emerald-600 border border-slate-100">
                            {p[0]}
                         </div>
