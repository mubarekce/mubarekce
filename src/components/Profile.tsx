...
const Profile: React.FC<ProfileProps> = ({ user, onUpdateUser, onLogout }) => {
  ...
  const renderNotifDetail = () => (
    <div className="flex-1 flex flex-col h-full bg-gradient-to-b from-white via-[#fbf6ea] to-[#f5ead0] dark:from-[#3e5878] dark:via-[#243a58] dark:to-[#141a2c] animate-in slide-in-from-right duration-500 overflow-hidden">
       {/* Compact Header */}
       <div className="px-6 pt-12 pb-4 flex items-center gap-5 border-b border-slate-50 dark:border-navy-800 bg-[#fbf6ea]/80 dark:bg-[#141a2c]/80 backdrop-blur-md sticky top-0 z-30">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-gold-500 uppercase tracking-[0.2em] mt-1">Gelişmiş Ses Kütüphanesi</p>
          </div>
       </div>
                            ...
                            return (
                               ...
                               >
                                  ...
                                  <div className="flex items-center gap-2">
                                     ...
                                     {isSelected && (
                                        <div className="w-5 h-5 bg-gold-500 rounded-full flex items-center justify-center text-white shadow-sm animate-in zoom-in duration-300">
  ...
  const renderMainView = () => (
    <div className="flex-1 overflow-y-auto px-6 pt-12 pb-40 space-y-8 bg-gradient-to-b from-white via-[#fbf6ea] to-[#f5ead0] dark:from-[#3e5878] dark:via-[#243a58] dark:to-[#141a2c] transition-colors duration-300 no-scrollbar relative animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <div className="w-10 h-10 bg-gold-50 dark:bg-navy-950/30 rounded-xl flex items-center justify-center border border-gold-100 dark:border-navy-800">
           <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="text-gold-600"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
        </div>
      </div>
      ...
      >
        {/* Cami Silüeti */}
        <div className={`absolute right-[-10px] bottom-0 pointer-events-none transition-transform duration-700 group-active:scale-105 group-active:translate-y-[-2px] ${isPremium ? 'opacity-[0.12] text-white' : 'opacity-[0.06] text-gold-600 dark:text-gold-400'}`}>
        ...
        <div className="relative z-10 h-full flex flex-col justify-between">
          <div className="flex justify-between items-start">
            ...
            {isPremium ? (
              <div className="bg-amber-400 text-navy-950 text-[9px] font-black px-3.5 py-1.5 rounded-full uppercase tracking-widest shadow-lg shadow-black/20 flex items-center gap-1.5 border border-white/20">
                <span className="w-1.5 h-1.5 bg-navy-900 rounded-full animate-pulse"></span>PREMİUM ÜYE
              </div>
            ) : (
              <div className="bg-gold-600/10 dark:bg-gold-400/10 backdrop-blur-md text-gold-700 dark:text-gold-300 text-[9px] font-black px-3.5 py-1.5 rounded-full uppercase tracking-widest flex items-center gap-1.5 border border-gold-200 dark:border-navy-800">STANDART</div>
            )}
          </div>
          
          <div className="space-y-0.5">
            <h3 className="text-xl font-black tracking-tight uppercase leading-none truncate">{user.name}</h3>
            <p className={`text-[10px] font-medium tracking-widest truncate ${isPremium ? 'text-gold-100/80' : 'text-gold-700/60 dark:text-gold-300/60'}`}>{user.email}</p>
            {user.bio && (
              <p className={`text-[11px] font-medium italic truncate pt-0.5 ${isPremium ? 'text-white/70' : 'text-navy-900/50 dark:text-gold-200/50'}`}>{user.bio}</p>
            )}
            {isPremium && (
      ...
      <div 
        ...
        className="bg-gradient-to-br from-navy-800 to-navy-950 p-6 rounded-[2.5rem] shadow-xl shadow-navy-900/10 text-white flex items-center justify-between gap-4 animate-in slide-in-from-left duration-700 delay-150 border border-white/10 relative overflow-hidden group cursor-pointer active:scale-95 transition-transform"
      >
        ...
        <div className="space-y-1 relative z-10 flex-1">
           ...
           <p className="text-[10px] font-medium text-gold-300/80 leading-tight mt-1.5">GPS destekli akıllı sessiz modu keşfedin.</p>
        </div>
      ...
      {showPremiumPage && (
        <div className="fixed inset-0 z-[1000] bg-gradient-to-b from-white via-[#fbf6ea] to-[#f5ead0] dark:from-[#3e5878] dark:via-[#243a58] dark:to-[#141a2c] flex flex-col h-full overflow-hidden pb-[max(1.5rem,env(safe-area-inset-bottom))] animate-in slide-in-from-bottom duration-500">
           {/* Success Overlay */}
           {purchaseSuccess && (
             <div className="absolute inset-0 bg-gold-700 z-[1100] flex flex-col items-center justify-center text-white p-10 animate-in fade-in zoom-in duration-500 text-center">
               ...
               <p className="text-gold-100 text-sm font-bold leading-relaxed mb-12">Artık Mübarekçe Pro+ üyesisiniz. Tüm özellikler hesabınıza tanımlandı.</p>
               <button onClick={() => { setShowPremiumPage(false); setPurchaseSuccess(false); }} className="w-full py-5 bg-white text-gold-700 font-black rounded-[2rem] text-xs uppercase tracking-[0.2em] shadow-2xl active:scale-95 transition-all">KİMLİK KARTIMI GÖSTER</button>
             </div>
           )}
           ...
           <div className="px-6 flex-1 flex flex-col justify-center gap-2 py-4">
              ...
              <div onClick={() => setSelectedPlan('3m')} className={`h-[88px] rounded-[1.8rem] border-2 transition-all cursor-pointer flex items-center justify-between px-5 ${selectedPlan === '3m' ? 'bg-[#1c2541] border-[#1c2541] shadow-xl' : 'bg-white dark:bg-navy-800 border-slate-100 dark:border-navy-900 shadow-sm'}`}>
                <div className="space-y-0.5">
  ...
  const renderMosqueDetail = () => (
    <div className="flex-1 flex flex-col h-full bg-gradient-to-b from-white via-[#fbf6ea] to-[#f5ead0] dark:from-[#3e5878] dark:via-[#243a58] dark:to-[#141a2c] animate-in slide-in-from-right duration-500">
       <div className="px-6 pt-12 pb-6 flex items-center gap-5 border-b border-slate-100 dark:border-navy-900">
          ...
          <div>
            ...
            <p className="text-[9px] font-black text-gold-500 uppercase tracking-[0.2em] mt-1">Akıllı Cami Algoritması</p>
          </div>
       </div>
          ...
          <div className="bg-[#1c2541] rounded-[3rem] p-10 text-white relative overflow-hidden shadow-2xl shadow-navy-900/20">
             ...
             <div className="relative z-10 space-y-8">
                <div className="space-y-2">
                   ...
                   <p className="text-[11px] font-bold text-gold-400 uppercase tracking-widest italic opacity-80">"Namaz müminin miracıdır."</p>
                </div>
