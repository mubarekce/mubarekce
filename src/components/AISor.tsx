      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              {['Namazın şartları nelerdir?', 'Sabah namazı kaç rekat?', 'Zikir çekmenin fazileti nedir?'].map(q => (
                <button 
                  key={q} 
                  onClick={() => setInput(q)}
                  className="bg-white border border-teal-100 text-teal-700 text-xs px-3 py-2 rounded-full hover:bg-teal-50 transition-colors"
                >
                  {q}
                </button>
              ))}
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-3 rounded-2xl shadow-sm text-sm ${m.role === 'user' ? 'bg-teal-600 text-white rounded-tr-none' : 'bg-white border border-gray-100 text-gray-800 rounded-tl-none'}`}>
              {m.text}
            </div>
          </div>
        ))}
