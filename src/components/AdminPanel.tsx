      ...
      <div className="p-5 space-y-8 overflow-y-auto flex-1">
        ...
        {tab === 'bolumler' && (
          <div className="space-y-6">
            ...
            {LIBRARY_CATEGORIES.map(cat => (
              <div key={cat}>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-3 ml-1">{cat}</p>
                <div className="bg-white rounded-[1.5rem] border border-slate-100 divide-y divide-slate-50 overflow-hidden">
        ...
        {tab === 'kullanicilar' && (
          <div className="space-y-4">
            <input
              ...
              className="w-full bg-white border border-slate-100 rounded-2xl px-4 py-3 text-sm outline-none"
            />
            {usersLoading && <p className="text-center text-xs text-slate-400 py-10">Kullanıcılar yükleniyor…</p>}
            {usersError && (
              <div className="p-4 bg-rose-50 text-rose-600 text-xs font-bold rounded-2xl">
                {usersError}
            ...
            {!usersLoading && !usersError && (
              <div className="bg-white rounded-[1.5rem] border border-slate-100 divide-y divide-slate-50 overflow-hidden">
                {filteredUsers.length === 0 && <p className="text-center text-xs text-slate-300 py-10">Kullanıcı bulunamadı</p>}
                {filteredUsers.map(u => (
                  <div key={u.uid} className="p-4 px-5 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-slate-800 truncate">{u.name}</p>
                      <p className="text-[11px] text-slate-400 truncate">{u.email || u.uid}</p>
                    </div>
                    <button
                      onClick={() => togglePremium(u)}
                      className={`shrink-0 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wide ${u.isPremium ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}
                    >
