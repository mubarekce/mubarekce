...
const FridayMessages: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-[#fdfdfd] animate-in fade-in duration-500 overflow-hidden relative">
      ...
      <div className="px-5 py-2 bg-[#faf6f0]/80 dark:bg-[#0d1220]/80 sticky top-[66px] z-30 overflow-x-auto no-scrollbar flex gap-1.5 border-b border-slate-50 dark:border-slate-800">
        {CATEGORIES.map(cat => (
          <button 
            key={cat.id}
